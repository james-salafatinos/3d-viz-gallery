...
var db = {}
let dotPlot = function (DATA, SCALE) {
    // console.log(DATA.nodes.length)
    // console.log(DATA.nodes[1]['x'])
    let vertices = [];
    ...
    for (let i = 0; i <= DATA.nodes.length; i += 1) {
        try {
            ...
            db[DATA.nodes[i]['label']] = { x: DATA.nodes[i]['x'], y: DATA.nodes[i]['y'] }
...
let edgePlot = function (DATA, scaler) {
    const points = [];
    // console.log(db)
    // console.log(db['1 Ceti'])
    console.log(DATA.edges.length)
    console.log(parseInt(db[DATA.edges[2]['source']]['x']))
